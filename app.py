...
# **📌 Sidebar - Pilih Sheet**
selected_sheet = st.sidebar.selectbox("📄 Pilih Sheet:", list(sheets_dict.keys()))

# **📌 Ambil data dari sheet yang dipilih**
df = sheets_dict[selected_sheet]

# **📌 Pastikan kolom memiliki nama yang benar**
df.columns = ["Ticket Number", "Created Date", "Condition", "Assign To"]

# **📌 Konversi 'Created Date' ke format datetime (tanpa jam)**
df["Created Date"] = pd.to_datetime(df["Created Date"], format="%d/%m/%Y").dt.date
